...
travellingSalesman paths toSee = minimum' $ catMaybes   $ map (travel toSee) (Map.keys paths )
...
part2 :: [String] -> Int
part2 xs = (-1) * ( fromJust  $ (travellingSalesman ( Map.map (map rev) $ makeGraph xs) ) (requiredCities xs) )